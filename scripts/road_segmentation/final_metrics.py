...
def get_tag(row):
        ...
        det_class=row.cover_type
        gt_class=row.CATEGORY

        if det_class=='undetermined' or det_class=='undetected':
            return 'FN'
        elif det_class==gt_class:
            return 'TP'
        elif det_class!=gt_class:
             return 'wrong class'
        else:
...
if __name__ == "__main__":
    ...
    predictions['det_class_name']=predictions.apply(lambda row: determine_class.get_corresponding_class(row, labels_id),
                                                        axis=1)
    ...
    visible_ground_truth=determine_class.clip_labels(filtered_ground_truth, considered_tiles[['title', 'id', 'geometry']].copy())
    ...
    if 'oth' in PREDICTIONS.keys():
        ...
        oth_predictions=predicted_roads_filtered[predicted_roads_filtered['dataset']=='oth'].copy()
        ground_truth_from_oth=filtered_ground_truth[filtered_ground_truth['gt_type']=='oth'].copy()
